...
- `Sinks`, `generate`, `create`는 **직접 onNext / onComplete / onError 같은 시그널을 푸시할 수 있게 해주는 도구**에요.
- 이 signal은 `Subscriber`에게 흘러가는 데이터의 이벤트 단위이자 알림인 거죠.

---

### 💡 비유로 이해하기

생각해봐요. 너가 **배달 기사**고, 너한테 오는 **전화**들이 signal이라고 해보자.
